    ...
    docs: {
      description: {
        component: [
          ...
          '',
          'A flagged duration is not color-only: `intent` also renders it',
          '**semibold**, so it stays findable in a column of plain durations',
          'without relying on hue. What *is* hue-only is telling one flagged',
          'intent from another — `risk` vs `breach` differ only in color. The',
          'usage contract, same as DataTable\'s `rowIntent`: pair a colored',
          'duration with text that says why. In the app, a breach-colored wait',
          'sits above a caption reading "4m over target", so the condition is',
          'stated, not merely painted.',
        ].join('\n'),
      },
    },
  },
